...
def generateQuery(num_queries, unit_num, batch_num, query_points, time_on_chain, time_low, time_high, device_value_max):
    ...
    query_time_length_mu = ((1 + time_on_chain) / 6)  # 尽可能让 time_length 较长  on_chain: 20  (21 / 6) = 3
    ...
    for _ in range(num_queries):
        ...
        while (True):
            ...
            device_length = round(unit_num * batch_num / time_length)  # 尽可能使得 query 内的 数据点数刚好为 1
            ...
            if 1 <= area <= query_points + 5 and is_overlap_points_less_enough(time_lower_bound, time_upper_bound,
                                                                               ...
                                                                               queries, int(unit_num * batch_num / 10)):
...
def getBasicBatch(time_max, time_on_chain, device_value_max):
...
def getRatioCutMethod(time_max, time_on_chain, unit_num, batch_num, device_value_max, query_points):
    ...
    for start_time in range(0, time_max, time_on_chain):
        ...
        if unit_num * batch_num == 1:
            ...
            query_set = generateQuery(num_queries, unit_num, batch_num, query_points, time_on_chain, start_time, end_time - 1, device_value_max)
...
def getFoundInBatch(Query_sets, time_on_chain, device_value_max, unit_num, batch_num, batched_basic_method, batched_proposed_method):
...
def getResult(found_in_basic_node, found_in_proposed_node, store_node_method_batched_basic_method,
              store_node_method_batched_proposed_method):
    time_consumed_max_basic, refuse_cnt_basic, accept_cnt_basic, serve_prob_basic\
        = cal_time(found_in_basic_node, store_node_method_batched_basic_method)
    time_consumed_max_proposed, refuse_cnt_proposed, accept_cnt_proposed, serve_prob_proposed\
        = cal_time(found_in_proposed_node, store_node_method_batched_proposed_method)
...
def writeToDisk(results, unit_num, batch_num, query_points):
    ...
    output_file = f'./0715/output_batch_size_{unit_num * batch_num}_{query_points}_0715.xlsx'
...
def main():
    ...
    for query_points in query_points_list:
        for i in range(len(unit_batch_num_list)):
            ...
            # Basic Method
            batched_basic_method = getBasicBatch(time_max, time_on_chain, device_value_max)
            # RatioCut Method
            ...
            # Get Batch 只需要执行一次
            found_in_basic_batch, found_in_proposed_batch, batches_sum_basic, batches_sum_proposed = getFoundInBatch(
                query_sets, time_on_chain, device_value_max, unit_num, batch_num, batched_basic_method,
                batched_proposed_method)
            ...
            (time_consumed_max_basic_r_d, refuse_cnt_basic_r_d, accept_cnt_basic_r_d, serve_prob_basic_r_d,
             time_consumed_max_proposed_r_d, refuse_cnt_proposed_r_d, accept_cnt_proposed_r_d, serve_prob_proposed_r_d)\
            ...
            results[str(unit_num * batch_num)].append((time_consumed_max_basic_r_d, refuse_cnt_basic_r_d, accept_cnt_basic_r_d, serve_prob_basic_r_d))
            results[str(unit_num * batch_num)].append((time_consumed_max_proposed_r_d, refuse_cnt_proposed_r_d, accept_cnt_proposed_r_d, serve_prob_proposed_r_d))
